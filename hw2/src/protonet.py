...
class ProtoNet:
    ...
    def _step(self, task_batch, N, K, Q):
        ...
        for task in task_batch:
            ...
            # ********************************************************
            # ******************* YOUR CODE HERE *********************
            # c_n_f = self._network(images_support).view(N, K, -1).mean(dim=1)
            s_fx_nq_f = self._network(images_support)
            c_n_f = s_fx_nq_f.view(N, K, -1).mean(dim=1)

            s_distance_as_logits = -torch.cdist(s_fx_nq_f, c_n_f, p=2)

            q_fx_nq_f = self._network(images_query)
            q_distance_as_logits = -torch.cdist(q_fx_nq_f, c_n_f, p=2)

            q_log_softmax = q_distance_as_logits - torch.logsumexp(q_distance_as_logits, dim=1).unsqueeze(-1)
            q_log_p = torch.gather(q_log_softmax, 1, labels_query.unsqueeze(-1))
            loss = -torch.mean(q_log_p)

            loss_batch.append(loss)
            accuracy_support_batch.append(util.score(s_distance_as_logits, labels_support))
            accuracy_query_batch.append(util.score(q_distance_as_logits, labels_query))
            # ********************************************************
        return (
    ...
    def log_softmax(self, d):
        ...
        return d - torch.logsumexp(d, dim=1)
    ...
    def distance(self, x, c):
        ...
        """
        z = self._network(x)
        return torch.cdist(z, c, p=2)

    def train(self, dataloader_train, dataloader_val, writer, N, K, Q):
        """Train the ProtoNet.
        ...
        for i_step, task_batch in enumerate(
            ...
            loss, accuracy_support, accuracy_query = self._step(task_batch, N, K, Q)
            loss.backward()
                ...
                with torch.no_grad():
                    ...
                    for val_task_batch in dataloader_val:
                        loss, accuracy_support, accuracy_query = (
                            self._step(val_task_batch, N, K, Q)
                        )
    ...
    def test(self, dataloader_test, N, K, Q):
        ...
        for task_batch in dataloader_test:
            accuracies.append(self._step(task_batch, N, K, Q)[2])
