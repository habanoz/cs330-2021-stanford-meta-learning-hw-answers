...
class MAML:
    ...
    def _inner_loop(self, images, labels, train):
        ...
        for _ in range(self._num_inner_steps):
            ...
            # compute gradients
            _param_keys = parameters.keys()
            _param_vals = [parameters[k] for k in _param_keys]
            grads = autograd.grad(loss, inputs=_param_vals, create_graph=train)

            # apply gradients
            for grad, param_key in zip(grads, _param_keys):
                parameters[param_key] = parameters[param_key] - self._inner_lrs[param_key] * grad
    ...
    def _outer_step(self, task_batch, train):
        ...
        for task in task_batch:
            ...
            accuracy = util.score(logits, labels_query)
